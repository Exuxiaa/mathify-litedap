...
Credentials and plan prices come from a gitignored `.env` at the project root, read by `com.mathify.util.MidtransConfig` (`MIDTRANS_SERVER_KEY`, `MIDTRANS_CLIENT_KEY`, `MIDTRANS_MERCHANT_ID`, `MIDTRANS_IS_PRODUCTION`, `MIDTRANS_PRICE_MONTHLY`, `MIDTRANS_PRICE_YEARLY`); OS env vars override the file.
With Snap the server sends the amount per transaction; `PremiumCheckoutServlet.resolvePlan` reads the monthly/yearly amounts from `MidtransConfig` (which fall back to defaults if the env vars are absent).
